...
void gameover(){
  textSize(60);
    fill(255,0,0);
  text("gameover",120,330);
//  stroke(255);
  fill(255,0,0);
  text("score:"+score,150,430);
...
void setup(){
  ...
  block=new Block(img,60,25,40,50); //圖片,磚塊長,高,邊界寬,高
  xBulSpeed=10;
  yBulSpeed=10;
  xBul=250;
  yBul=250;
  ...
  for(int i=0; i<7; i++){
    for(int j=0; j<7; j++){
      if(a[j][i] >=1 && a[j][i] <= 3){
        x=40+i*60;;
        y=50+j*20;
//        if( (yBulSpeed>0 && xBul-10 >= x && xBul+10 <= x+60 && yBul-10 >= y && yBul+10 <= y+20)
//        || (yBulSpeed<0 && xBul-10 >= x && xBul+10 <= x+60 && yBul-10 >= y && yBul+10 <= y+20)){
        if( (yBulSpeed>0 && xBul-10 >= x && xBul+10 <= x+60 && yBul >= y && yBul+10 <= y+20)
        || (yBulSpeed<0 && xBul-10 >= x && xBul+10 <= x+60 && yBul >= y+20 && yBul+10 <= y+30)){
           yBulSpeed= -1 *yBulSpeed;
           if(over==0)
             score+=5;//得分
           a[j][i]=0;
...
//    if ((xBul-10) >= xPad && (xBul+10) <= (xPad+70) && (yBul-10) >= yPad && (yBul+10) < (yPad+20)){
 if ((xBul-10) >= xPad && (xBul+10) <= (xPad+70) && (yBul+10) >= yPad && (yBul+20) < (yPad+30)){
          yBulSpeed= -1 *yBulSpeed;
    }    
    image(pad, xPad, yPad,70,30);
    if(ball <=0){
      over=1;
      gameover();
    }
...
void mouseMoved(){
  if(over==0 && mouseX >= xPad+70){
    xPad=xPad+10;
  }
  if(over==0 && mouseX < xPad){
    xPad=xPad-10;
  }
  if(xPad+70>width-40){
      xPad=width-40-70;//防止球拍超過右邊界
